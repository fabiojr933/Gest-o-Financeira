...
function getMesInicioFim($ano, $mes) {
    $primeiro = date("Y-m-d", strtotime("$ano-$mes-01"));
    $ultimo = date("Y-m-d", strtotime("last day of $ano-$mes-01"));
...
function validarDatas($datas) {
    ...
    if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $datas['inicio'])) {
        return false;
    }

    if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $datas['fim'])) {
        return false;
    }

    // validar se são datas reais
    list($y1, $m1, $d1) = explode('-', $datas['inicio']);
    if (!checkdate($m1, $d1, $y1)) {
        return false;
    }

    list($y2, $m2, $d2) = explode('-', $datas['fim']);
